...
import com.fresh.common.util.Const;
import com.fresh.common.util.Tools;
...
public class Page {
    ...
    public String getPageStr() {
        ...
        if (totalResult > 0) {
            ...
            sb.append("	<li><select title='显示条数' style=\"width:55px;float:left;margin-top:1px;\" onchange=\"changeCount(this.value,this)\">\n");
            sb.append("	<option value='" + showCount + "'>" + (showCount > 100 ? "全部" : showCount) + "</option>\n");
            ...
            sb.append("	<option value='99999'>全部</option>\n");
            sb.append("	</select>\n");
            sb.append("	</li>\n");
            ...
            sb.append("function changeCount_confirmed(value){");
            sb.append(" top.jzts();");
            sb.append("	if(true && document.forms[0]){\n");
            sb.append("		var url = document.forms[0].getAttribute(\"action\");\n");
            sb.append("		if(url.indexOf('?')>-1){url += \"&" + (entityOrField ? "currentPage" : "page.currentPage") + "=\";}\n");
            ...
            sb.append("}\n");
            //调整每页显示条数
            sb.append("function changeCount(value, selCtrl){");
            sb.append(" if(value>100 && " + totalResult + " > 100 ){\n");
            sb.append("   bootbox.confirm(\"显示全部数据(共" + totalResult + "条)，可能会造成系统缓慢，真的要显示全部数据吗？\", function(result) {\n");
            sb.append("     if(result) {\n");
            sb.append("       changeCount_confirmed(value);\n");
            sb.append("     }\n");
            sb.append("     else{\n");
            sb.append("        selCtrl.options[0].selected = true;\n");
            sb.append("     }\n");
            sb.append("   });\n");
            sb.append(" }\n");
            sb.append(" else{\n");
